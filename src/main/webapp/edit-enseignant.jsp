...
<%
	...
	// Utilisateur u = listUser.get(pos);
	Enseignant u =	EnseignantDAO.findById(pos);
	%>
	...
	<div class="container">
		...
		<div class="card">
			<h5 class="card-header" style="display: flex;justify-content: center">Modifier Enseignant : &nbsp; <span style="font-weight: bold;"><%=u.getNom()%>&nbsp;<%=u.getPrenom()%></span></h5>
			<div class="card-body" >



				<form method="post" action="UpdateEnseignantController">

					<input type="hidden" name="pos" value="<%=pos%>">
					
					<div class="form-group">
						<label for="nom">Nom:</label> <input type="text"
							class="form-control" id="nom" required placeholder="Enter nom" name="nom" value="<%=u.getNom()%>"
							>
					</div>

					<div class="form-group">
						<label for="prenom">Prenom:</label> <input type="text"
							class="form-control" id="prenom" required placeholder="Enter prenom" value="<%=u.getPrenom()%>"
							name="prenom" >
					</div>

					<div class="form-group">
						<label for="email">Email:</label> <input type="email"
							class="form-control" id="email" required placeholder="Enter email" value="<%=u.getEmail()%>"
							name="email" >
					</div>

					<div class="form-group">
						<label for="pwd">Password:</label> <input class="form-control" value="<%=u.getPassword()%>"
							id="pwd" placeholder="Enter password" required name="password"
							>
					</div>


					<button type="submit" required class="btn btn-primary">Modifier</button>
				</form>
			</div>
		</div>
	</div>
